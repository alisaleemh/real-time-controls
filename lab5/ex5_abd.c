...
// Define the name of the MQ, use your
...
void *Sender(void *arg)  // This thread sends msgs to the MQ
{
    ...
    mqd_t tx;
    struct Msg myMsg;
    unsigned int msg_priority[10];  // Set up message priorities
    ...
    for (i = 1; i <= SIZE; i++) {
        sprintf(myMsg.str, "This is Test msg %d", i);  // Set up message to be sent
